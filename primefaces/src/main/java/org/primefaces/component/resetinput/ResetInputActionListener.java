...
public class ResetInputActionListener implements ActionListener {
    ...
    public ResetInputActionListener(final ValueExpression target) {
        this.target = target;
    }
    ...
    public void processAction(final ActionEvent event) throws AbortProcessingException {
        ...
        final String targetIds = (String) target.getValue(elContext);
        final UIComponent source = event.getComponent();

        final String[] ids = targetIds.split("[,\\s]+");
        for (final String id : ids) {
            final UIComponent targetComponent = source.findComponent(id);
