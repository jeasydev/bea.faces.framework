...
public class DataTableRenderer extends DataRenderer {
    ...
    public void decode(final FacesContext context, final UIComponent component) {
        ...
        for (final DataTableFeature feature : DataTable.FEATURES.values()) {
    ...
    protected void encodeCell(final FacesContext context,
        ...
        styleClass = userStyleClass == null ? styleClass : (styleClass == null) ? userStyleClass : styleClass + " "
            + userStyleClass;
    ...
    public void encodeColGroup(final FacesContext context, final DataTable table) throws IOException {
        final ResponseWriter writer = context.getResponseWriter();
    ...
    protected void encodeColumnFooter(final FacesContext context, final DataTable table, final UIColumn column)
        throws IOException {
        if (!column.isRendered()) {
            return;
        }
    ...
    protected void encodeColumnHeader(final FacesContext context, final DataTable table, final UIColumn column)
        throws IOException {
        if (!column.isRendered()) {
            return;
        }
        ...
        final ResponseWriter writer = context.getResponseWriter();
        final String clientId = column.getContainerClientId(context);
                ...
                if (table.isMultiSort()) {
                    ...
                    if (sortMeta != null) {
                        for (final SortMeta meta : sortMeta) {
                            sortIcon = resolveDefaultSortIcon(columnSortByVe, meta.getColumn()
                                .getValueExpression("sortBy"), meta.getSortOrder().name());
        ...
        if (width != null) {
            final String unit = width.endsWith("%") ? "" : "px";
            if (style != null)
    ...
    protected void encodeColumnHeaderContent(final FacesContext context, final UIColumn column, final String sortIcon)
        throws IOException {
        final ResponseWriter writer = context.getResponseWriter();
    ...
    protected void encodeColumnSelection(final FacesContext context,
                                         final DataTable table,
                                         final String clientId,
                                         final UIColumn column,
                                         final boolean selected) throws IOException {
        final String selectionMode = column.getSelectionMode();
        final boolean disabled = column.isDisabledSelection();
    ...
    protected void encodeFilter(final FacesContext context, final DataTable table, final UIColumn column)
        throws IOException {
        final Map<String, String> params = context.getExternalContext().getRequestParameterMap();
        final ResponseWriter writer = context.getResponseWriter();
        final String separator = String.valueOf(UINamingContainer.getSeparatorChar(context));

        final String filterId = column.getContainerClientId(context) + separator + "filter";
        ...
        if (table.isReset()) {
            ...
            if (params.containsKey(filterId)) {
                ...
                final ValueExpression filterValueVE = column.getValueExpression("filterValue");
    ...
    protected void encodeFrozenRows(final FacesContext context, final DataTable table) throws IOException {
        final Collection<?> frozenRows = table.getFrozenRows();
        ...
        final int index = 0;
        for (final Object object : frozenRows) {
    ...
    protected void encodeLiveRows(final FacesContext context, final DataTable table) throws IOException {
        final Map<String, String> params = context.getExternalContext().getRequestParameterMap();
    ...
    protected void encodeMarkup(final FacesContext context, final DataTable table) throws IOException {
        final ResponseWriter writer = context.getResponseWriter();
        final String clientId = table.getClientId(context);
        final boolean scrollable = table.isScrollable();
        final boolean hasPaginator = table.isPaginator();
        final String style = table.getStyle();
        final String paginatorPosition = table.getPaginatorPosition();
        ...
        if (!table.isDefaultSorted() && table.getValueExpression("sortBy") != null && !table.isLazy()) {
            final SortFeature sortFeature = (SortFeature) DataTable.FEATURES.get(DataTableFeatureKey.SORT);

            if (table.isMultiSort()) {
                sortFeature.multiSort(context, table);
            } else {
                sortFeature.sort(context, table, table.getValueExpression("sortBy"), table.convertSortOrder(), table
                    .getSortFunction());
            }
        ...
        if (table.isSelectionEnabled()) {
            encodeStateHolder(context, table, table.getClientId(context) + "_selection", table
    ...
    protected void encodeRadio(final FacesContext context,
                               final DataTable table,
                               final boolean checked,
    ...
    protected void encodeRegularTable(final FacesContext context, final DataTable table) throws IOException {
        final ResponseWriter writer = context.getResponseWriter();
    ...
    public boolean encodeRow(final FacesContext context,
                             final DataTable table,
                             final String clientId,
                             final int rowIndex,
                             final String rowIndexVar) throws IOException {
        final ResponseWriter writer = context.getResponseWriter();

        final boolean selectionEnabled = table.isSelectionEnabled();
        ...
        // Preselection
        final boolean selected = table.getSelectedRowKeys().contains(rowKey);

        final String userRowStyleClass = table.getRowStyleClass();
    ...
    protected void encodeRowExpansion(final FacesContext context, final DataTable table) throws IOException {
        final ResponseWriter writer = context.getResponseWriter();
        final Map<String, String> params = context.getExternalContext().getRequestParameterMap();
    ...
    protected void encodeScript(final FacesContext context, final DataTable table) throws IOException {
        final ResponseWriter writer = context.getResponseWriter();
        final String clientId = table.getClientId(context);
        final String selectionMode = table.resolveSelectionMode();
        ...
        if (table.isFilteringEnabled()) {
            wb.attr("filter", true).attr("filterEvent", table.getFilterEvent(), null).attr("filterDelay",
                                                                                           table.getFilterDelay(),
                                                                                           Integer.MAX_VALUE);
        }
        ...
        if (table.isScrollable()) {
            wb.attr("scrollable", true).attr("liveScroll", table.isLiveScroll()).attr("scrollStep",
                                                                                      table.getScrollRows())
        ...
        // Editing
        if (table.isEditable()) {
            wb.attr("editable", true).attr("editMode", table.getEditMode()).attr("cellSeparator",
                                                                                 table.getCellSeparator(), null);
        }
    ...
    protected void encodeScrollableTable(final FacesContext context, final DataTable table) throws IOException {
        final String tableStyle = table.getStyle();
        final String tableStyleClass = table.getStyleClass();

        encodeScrollAreaStart(context, table, DataTable.SCROLLABLE_HEADER_CLASS, DataTable.SCROLLABLE_HEADER_BOX_CLASS,
                              tableStyle, tableStyleClass);
        encodeThead(context, table);
        encodeScrollAreaEnd(context);

        encodeScrollBody(context, table, tableStyle, tableStyleClass);

        encodeScrollAreaStart(context, table, DataTable.SCROLLABLE_FOOTER_CLASS, DataTable.SCROLLABLE_FOOTER_BOX_CLASS,
                              tableStyle, tableStyleClass);
        encodeTFoot(context, table);
        encodeScrollAreaEnd(context);
    }
    ...
    protected void encodeScrollBody(final FacesContext context,
                                    ...
                                    final String tableStyleClass) throws IOException {
        final ResponseWriter writer = context.getResponseWriter();
        final String scrollHeight = table.getScrollHeight();
    ...
    protected void encodeSubTable(final FacesContext context,
                                  final DataTable table,
    ...
    protected void encodeSummaryRow(final FacesContext context, final DataTable table, final SummaryRow summaryRow)
        throws IOException {
        final MethodExpression me = summaryRow.getListener();
        if (me != null) {
    ...
    public void encodeTbody(final FacesContext context, final DataTable table, final boolean dataOnly)
        throws IOException {
        final ResponseWriter writer = context.getResponseWriter();
        final String rowIndexVar = table.getRowIndexVar();
        final String clientId = table.getClientId(context);
        final String emptyMessage = table.getEmptyMessage();
        final UIComponent emptyFacet = table.getFacet("emptyMessage");
        final SubTable subTable = table.getSubTable();
        final SummaryRow summaryRow = table.getSummaryRow();
        ...
        final int rowCountToRender = rows == 0 ? (table.isLiveScroll() ? table.getScrollRows() : rowCount) : rows;
        final boolean hasData = rowCount > 0;
    ...
    protected void encodeTFoot(final FacesContext context, final DataTable table) throws IOException {
        final ResponseWriter writer = context.getResponseWriter();
     ...
     * defined
     */
    protected void encodeThead(final FacesContext context, final DataTable table) throws IOException {
        final ResponseWriter writer = context.getResponseWriter();
        final ColumnGroup group = table.getColumnGroup("header");
    ...
    protected SelectItem[] getFilterOptions(final UIColumn column) {
        final Object options = column.getFilterOptions();
        ...
        if (options instanceof SelectItem[]) {
            ...
            throw new FacesException("Filter options for column " + column.getClientId()
    ...
    boolean isInSameGroup(final FacesContext context, final DataTable table, final int currentRowIndex) {
        context.getExternalContext().getRequestMap();
        table.getVar();

        table.setRowIndex(currentRowIndex);
        final Object currentGroupByData = table.getSortBy();

        table.setRowIndex(currentRowIndex + 1);
        if (!table.isRowAvailable()) return false;

        final Object nextGroupByData = table.getSortBy();
    ...
    protected String resolveDefaultSortIcon(final ValueExpression columnSortBy,
                                            final ValueExpression tableSortBy,
                                            final String sortOrder) {
        final String columnSortByExpression = columnSortBy.getExpressionString();
        final String tableSortByExpression = tableSortBy.getExpressionString();
        String sortIcon = null;
