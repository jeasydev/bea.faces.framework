...
public class InplaceRenderer extends CoreRenderer {
    ...
    protected void encodeEditor(final FacesContext context, final Inplace inplace) throws IOException {
        final ResponseWriter writer = context.getResponseWriter();
    ...
    public void encodeEnd(final FacesContext context, final UIComponent component) throws IOException {
        final Inplace inplace = (Inplace) component;
    ...
    protected void encodeMarkup(final FacesContext context, final Inplace inplace) throws IOException {
        ...
        final String styleClass = userStyleClass == null ? Inplace.CONTAINER_CLASS : Inplace.CONTAINER_CLASS + " "
            + userStyleClass;
        ...
        final boolean validationFailed = context.isValidationFailed();
        final String displayStyle = validationFailed ? "none" : "inline";
        final String contentStyle = validationFailed ? "inline" : "none";
    ...
    protected void encodeScript(final FacesContext context, final Inplace inplace) throws IOException {
        final ResponseWriter writer = context.getResponseWriter();
        final String clientId = inplace.getClientId(context);
        final WidgetBuilder wb = getWidgetBuilder(context);
        wb.widget("Inplace", inplace.resolveWidgetVar(), clientId, false).attr("effect", inplace.getEffect())
            .attr("effectSpeed", inplace.getEffectSpeed()).attr("event", inplace.getEvent()).attr("toggleable",
                                                                                                  inplace
                                                                                                      .isToggleable(),
                                                                                                  false)
            .attr("disabled", inplace.isDisabled(), false).attr("editor", inplace.isEditor(), false);
    ...
    protected String getLabelToRender(final FacesContext context, final Inplace inplace) {
        final String label = inplace.getLabel();
        final String emptyLabel = inplace.getEmptyLabel();

        if (label != null) {
            return label;
        } else {
            final String value = ComponentUtils.getValueToRender(context, inplace.getChildren().get(0));
