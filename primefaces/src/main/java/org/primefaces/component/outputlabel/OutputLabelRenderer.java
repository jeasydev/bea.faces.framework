...
public class OutputLabelRenderer extends CoreRenderer {
    ...
    public void encodeEnd(final FacesContext context, final UIComponent component) throws IOException {
        ...
        final String clientId = label.getClientId();
        final Object value = label.getValue();
        ...
        if (label.getFor() != null) {
            ...
            targetClientId = (target instanceof InputHolder) ? ((InputHolder) target).getInputClientId() : target
                .getClientId(context);
            ...
            if (target instanceof UIInput) {
                ...
                if (input.getAttributes().get("label") == null || input.getValueExpression("label") == null) {
                    final ValueExpression ve = label.getValueExpression("value");
                    if (ve != null)
                        input.setValueExpression("label", ve);
                    else input.getAttributes().put("label", value);
                }
    ...
    protected UIComponent findTarget(final FacesContext context, final OutputLabel label) {
        UIComponent _forComponent = null;
        final String _for = label.getFor();

        if (_for != null) {
            _forComponent = label.findComponent(_for);
            if (_forComponent == null) {
                throw new FacesException("Cannot find component with identifier \"" + _for + "\" referenced from \""
                    + label.getClientId(context) + "\".");
            }
        }
