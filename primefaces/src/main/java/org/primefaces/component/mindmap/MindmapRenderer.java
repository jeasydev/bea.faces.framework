...
public class MindmapRenderer extends CoreRenderer {
    ...
    public void encodeEnd(final FacesContext context, final UIComponent component) throws IOException {
        final Mindmap map = (Mindmap) component;

        if (map.isNodeSelectRequest(context)) {
            final MindmapNode node = map.getSelectedNode();
    ...
    protected void encodeMarkup(final FacesContext context, final Mindmap map) throws IOException {
        final ResponseWriter writer = context.getResponseWriter();
        final String clientId = map.getClientId(context);
        final String style = map.getStyle();
    ...
    protected void encodeNode(final FacesContext context,
        ...
        final ResponseWriter writer = context.getResponseWriter();
        final List<MindmapNode> children = node.getChildren();
        final MindmapNode parent = node.getParent();
        ...
        if (parent != null) {
            final String parentNodeKey = (nodeKey.indexOf("_") != -1)
                                                                     ? nodeKey.substring(0, nodeKey.lastIndexOf("_"))
                                                                     : "root";
        ...
        if (!children.isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                final String childKey = (nodeKey.equals("root")) ? String.valueOf(i) : nodeKey + "_" + i;

                final MindmapNode child = children.get(i);
    ...
    protected void encodeNodeConfig(final FacesContext context,
                                    final Mindmap map,
                                    final MindmapNode node,
                                    final String nodeKey) throws IOException {
        final ResponseWriter writer = context.getResponseWriter();
    ...
    protected void encodeScript(final FacesContext context, final Mindmap map) throws IOException {
        final ResponseWriter writer = context.getResponseWriter();
        final String clientId = map.getClientId(context);
        final MindmapNode root = map.getValue();
