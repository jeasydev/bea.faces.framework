...
public class WatermarkRenderer extends CoreRenderer {
    ...
    public void encodeEnd(final FacesContext context, final UIComponent component) throws IOException {
        ...
        final Watermark watermark = (Watermark) component;
        String target = null;

        if (watermark.getFor() != null) {
            final String _for = watermark.getFor();
            final UIComponent forComponent = watermark.findComponent(_for);
        ...
        wb.widget("Watermark", watermark.resolveWidgetVar(), watermark.getClientId(context), "watermark", true)
            .attr("value", watermark.getValue()).attr("target", target);
