...
public class TagCloudRenderer extends CoreRenderer {
    ...
    public void encodeEnd(final FacesContext context, final UIComponent component) throws IOException {
        final TagCloud tagCloud = (TagCloud) component;

        encodeMarkup(context, tagCloud);
        encodeScript(context, tagCloud);
    }

    protected void encodeMarkup(final FacesContext context, final TagCloud tagCloud) throws IOException {
        final ResponseWriter writer = context.getResponseWriter();
        final TagCloudModel model = tagCloud.getModel();
        String styleClass = tagCloud.getStyleClass();
        final String style = tagCloud.getStyle();
        ...
        for (final TagCloudItem item : model.getTags()) {
            final String url = item.getUrl();
            final String href = url == null ? "#" : getResourceURL(context, item.getUrl());
    ...
    protected void encodeScript(final FacesContext context, final TagCloud tagCloud) throws IOException {
        final ResponseWriter writer = context.getResponseWriter();
        final String clientId = tagCloud.getClientId(context);
