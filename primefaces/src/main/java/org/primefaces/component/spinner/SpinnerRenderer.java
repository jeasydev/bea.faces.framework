...
public class SpinnerRenderer extends InputRenderer {
    ...
    public void decode(final FacesContext context, final UIComponent component) {
        ...
        String submittedValue = context.getExternalContext().getRequestParameterMap().get(spinner.getClientId(context)
                                                                                              + "_input");
        final String prefix = spinner.getPrefix();
        final String suffix = spinner.getSuffix();
    ...
    protected void encodeButton(final FacesContext context, final String styleClass, final String iconClass)
        throws IOException {
    ...
    protected void encodeInput(final FacesContext context, final Spinner spinner) throws IOException {
        final ResponseWriter writer = context.getResponseWriter();
        final String inputId = spinner.getClientId(context) + "_input";
    ...
    protected void encodeMarkup(final FacesContext context, final Spinner spinner) throws IOException {
        final ResponseWriter writer = context.getResponseWriter();
        final String clientId = spinner.getClientId(context);
    ...
    protected void encodeScript(final FacesContext context, final Spinner spinner) throws IOException {
        final ResponseWriter writer = context.getResponseWriter();
        final String clientId = spinner.getClientId(context);
        final WidgetBuilder wb = getWidgetBuilder(context);
        wb.widget("Spinner", spinner.resolveWidgetVar(), clientId, false).attr("step", spinner.getStepFactor())
            .attr("min", spinner.getMin(), Double.MIN_VALUE).attr("max", spinner.getMax(), Double.MAX_VALUE)
            .attr("prefix", spinner.getPrefix(), null).attr("suffix", spinner.getSuffix(), null);
