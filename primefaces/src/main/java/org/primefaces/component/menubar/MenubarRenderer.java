...
public class MenubarRenderer extends TieredMenuRenderer {
    ...
    protected void encodeMarkup(final FacesContext context, final AbstractMenu abstractMenu) throws IOException {
        final Menubar menubar = (Menubar) abstractMenu;
        final String style = menubar.getStyle();
    ...
    protected void encodeScript(final FacesContext context, final AbstractMenu abstractMenu) throws IOException {
        ...
        final Menubar menubar = (Menubar) abstractMenu;
        final String clientId = menubar.getClientId(context);

        final WidgetBuilder wb = getWidgetBuilder(context);
        wb.widget("Menubar", menubar.resolveWidgetVar(), clientId, false).attr("autoDisplay", menubar.isAutoDisplay());
    ...
    protected void encodeSubmenuIcon(final FacesContext context, final Submenu submenu) throws IOException {
        final ResponseWriter writer = context.getResponseWriter();
        final String icon = submenu.getParent() instanceof Menubar
                                                                  ? AbstractMenu.SUBMENU_DOWN_ICON_CLASS
                                                                  : AbstractMenu.SUBMENU_RIGHT_ICON_CLASS;
