...
public class PieChartRenderer extends BaseChartRenderer {
    ...
    protected void encodeData(final FacesContext context, final PieChart chart) throws IOException {
        final ResponseWriter writer = context.getResponseWriter();

        writer.write(",data:[[");
        final PieChartModel model = (PieChartModel) chart.getValue();

        for (final Iterator<String> it = model.getData().keySet().iterator(); it.hasNext();) {
            final String key = it.next();
            final Number value = model.getData().get(key);
    ...
    public void encodeEnd(final FacesContext context, final UIComponent component) throws IOException {
        final PieChart chart = (PieChart) component;

        encodeMarkup(context, chart);
        encodeScript(context, chart);
    }

    protected void encodeOptions(final FacesContext context, final PieChart chart) throws IOException {
        super.encodeOptions(context, chart);

        final ResponseWriter writer = context.getResponseWriter();

        if (chart.getDiameter() != Integer.MIN_VALUE) writer.write(",diameter:" + chart.getDiameter());
    ...
    protected void encodeScript(final FacesContext context, final UIChart uichart) throws IOException {
        final ResponseWriter writer = context.getResponseWriter();
        final PieChart chart = (PieChart) uichart;
