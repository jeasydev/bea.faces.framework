...
public class RowsPerPageDropdownRenderer implements PaginatorElementRenderer {
    ...
    public void render(final FacesContext context, final UIData uidata) throws IOException {
        final String template = uidata.getRowsPerPageTemplate();

        if (template != null) {
            final ResponseWriter writer = context.getResponseWriter();
            final int actualRows = uidata.getRows();
            final String[] options = uidata.getRowsPerPageTemplate().split("[,\\s]+");
            ...
            for (final String option : options) {
                final int rows = Integer.parseInt(option);
