...
public class UINotificationRenderer extends CoreRenderer {
    ...
    protected String getSeverityName(final FacesMessage message) {
        ...
        if (ordinal == FacesMessage.SEVERITY_INFO.getOrdinal())
            ...
            severity = "warn";
        else if (ordinal == FacesMessage.SEVERITY_FATAL.getOrdinal()) severity = "fatal";

        return severity;
    }

    protected boolean shouldRender(final UINotification component, final FacesMessage message, final String severityName) {
        final String severityLevel = component.getSeverity();

        if ((message.isRendered() && !component.isRedisplay())
            || (severityLevel != null && severityLevel.indexOf(severityName) == -1)) {
            return false;
        } else {
            return true;
        }
