...
public class JsonConverter implements Converter, Serializable {
    ...
    static {
        JsonConverter.PRIMITIVE_CLASSES.put("boolean", boolean.class);
        JsonConverter.PRIMITIVE_CLASSES.put("byte", byte.class);
        JsonConverter.PRIMITIVE_CLASSES.put("short", short.class);
        JsonConverter.PRIMITIVE_CLASSES.put("char", char.class);
        JsonConverter.PRIMITIVE_CLASSES.put("int", int.class);
        JsonConverter.PRIMITIVE_CLASSES.put("long", long.class);
        JsonConverter.PRIMITIVE_CLASSES.put("float", float.class);
        JsonConverter.PRIMITIVE_CLASSES.put("double", double.class);

        JsonConverter.PRIMITIVE_ARRAY_CLASSES.put("boolean[]", boolean[].class);
        JsonConverter.PRIMITIVE_ARRAY_CLASSES.put("byte[]", byte[].class);
        JsonConverter.PRIMITIVE_ARRAY_CLASSES.put("short[]", short[].class);
        JsonConverter.PRIMITIVE_ARRAY_CLASSES.put("char[]", char[].class);
        JsonConverter.PRIMITIVE_ARRAY_CLASSES.put("int[]", int[].class);
        JsonConverter.PRIMITIVE_ARRAY_CLASSES.put("long[]", long[].class);
        JsonConverter.PRIMITIVE_ARRAY_CLASSES.put("float[]", float[].class);
        JsonConverter.PRIMITIVE_ARRAY_CLASSES.put("double[]", double[].class);
    }
    ...
    protected java.lang.reflect.Type getObjectType(final String type, final boolean isTypeArg) {
        Class clazz = JsonConverter.PRIMITIVE_CLASSES.get(type);
        if (clazz != null) {
            if (!isTypeArg) {
                return clazz;
            } else {
                throw new ConverterException(new FacesMessage(FacesMessage.SEVERITY_ERROR,
                                                              "Type argument can not be a primitive type, but it was "
                                                                  + type + ".",
                                                              StringUtils.EMPTY));
            }
        }

        clazz = JsonConverter.PRIMITIVE_ARRAY_CLASSES.get(type);
        if (clazz != null) {
            return clazz;
        }
        ...
        if (arrayBracketIdx >= 0 && (leftBracketIdx < 0 || arrayBracketIdx < leftBracketIdx)) {
            ...
            try {
                ...
                throw new ConverterException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "Class "
                    + type.substring(0, arrayBracketIdx) + " not found", StringUtils.EMPTY));
            }
        }
        ...
        if (leftBracketIdx < 0) {
            try {
                ...
                throw new ConverterException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "Class " + type
        ...
        if (rightBracketIdx < 0) {
            throw new ConverterException(new FacesMessage(FacesMessage.SEVERITY_ERROR, type
                + " is not a valid generic type.", StringUtils.EMPTY));
        }
        ...
        try {
            ...
            throw new ConverterException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "Class "
                + type.substring(0, leftBracketIdx) + " not found", StringUtils.EMPTY));
        }

        final String strTypeArgs = type.substring(leftBracketIdx + 1, rightBracketIdx);
        final List<String> listTypeArgs = new ArrayList<String>();
        int startPos = 0;
        int seekPos = 0;
