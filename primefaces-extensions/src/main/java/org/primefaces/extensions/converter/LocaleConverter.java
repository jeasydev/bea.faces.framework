...
/**
 * {@link Converter} which converts a string to a {@link java.util.Locale} an
 * vice-versa.
 * 
...
public class LocaleConverter implements Converter, Serializable {
    ...
    public static Locale getLocaleObject(final String locale, final char seperator) {
        ...
        if (parts.length == 0 || !parts[0].matches("[a-zA-Z]{2,2}")
            || (parts.length > 1 && parts[1].length() != 0 && !parts[1].matches("[a-zA-Z]{2,2}"))) {
            throw new ConverterException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "'" + locale
                + "' does not represent a valid locale", StringUtils.EMPTY));
        }
        ...
        switch (parts.length) {
            ...
            default:
                return new Locale(parts[0], parts[1], StringUtils
                    .join(ArrayUtils.subarray(parts, 2, parts.length), '_'));
        }
    }
    ...
    public Object getAsObject(final FacesContext fc, final UIComponent component, final String value) {
        ...
        return LocaleConverter.getLocaleObject(value, separator);
    }
    ...
    public String getAsString(final FacesContext fc, final UIComponent component, final Object value) {
        if (value == null) {
            ...
            return LocaleConverter.getLocaleString(defaultLocale, separator);
        }

        if (value instanceof String) {
            return (String) value;
        } else if (value instanceof Locale) {
            return LocaleConverter.getLocaleString((Locale) value, separator);
        } else {
            throw new ConverterException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "Wrong type: '"
                + value.getClass().getSimpleName() + "' is not 'Locale'.", StringUtils.EMPTY));
        }
    }
