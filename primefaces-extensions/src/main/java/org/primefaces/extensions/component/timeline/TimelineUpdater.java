...
public abstract class TimelineUpdater {
     ...
     * @throws FacesException if the Timeline component can not be found by the
     *             given Id
     */
    public static TimelineUpdater getCurrentInstance(final String id) {
        final FacesContext fc = FacesContext.getCurrentInstance();

        @SuppressWarnings("unchecked")
        final Map<String, TimelineUpdater> map = (Map<String, TimelineUpdater>) fc.getAttributes()
            .get(TimelineUpdater.class.getName());
        if (map == null) {
            return null;
        }

        final UIComponent timeline = fc.getViewRoot().findComponent(id);
        if (timeline == null || !(timeline instanceof Timeline)) {
            throw new FacesException("Timeline component with Id " + id + " was not found");
        }

        final TimelineUpdater timelineUpdater = map.get(((Timeline) timeline).resolveWidgetVar());
     ...
     * {@link #getCurrentInstance(String)}
     */
    protected String id;
