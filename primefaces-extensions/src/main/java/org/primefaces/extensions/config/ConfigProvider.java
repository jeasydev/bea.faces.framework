...
public class ConfigProvider {
    ...
    public static void cleanCache() {
        ConfigProvider.CACHE.remove();
    }

    /**
     * Gets the one and only {@link ConfigContainer} instance:
     * 
     * - try to lookup it from a {@link ThreadLocal} cache - try to get it from
     * the application map - create a new instance, store it in the
     * {@link ThreadLocal} cache and in the application map
     * 
     ...
     * 
     * @param context The {@link FacesContext}.
     * @param If the {@link ConfigContainer} should be cached in the
    ...
    public static ConfigContainer getConfig(final FacesContext context, final boolean cacheInThreadLocal) {
        ...
        if (cacheInThreadLocal) {
            ...
            container = ConfigProvider.CACHE.get();
            if (container == null) {
                container = ConfigProvider.getConfigFromApplicationMap(context);
                ConfigProvider.CACHE.set(container);
            }
    ...
    private static ConfigContainer getConfigFromApplicationMap(final FacesContext context) {
        ...
        ConfigContainer container = (ConfigContainer) context.getExternalContext().getApplicationMap()
            .get(ConfigProvider.KEY);

        if (container == null) {
            container = new ConfigContainer(context);

            context.getExternalContext().getApplicationMap().put(ConfigProvider.KEY, container);
        }
