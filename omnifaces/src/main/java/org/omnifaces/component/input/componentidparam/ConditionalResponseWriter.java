 ...
 * {@link ComponentIdParam} component.
 * <p>
 * This allows rendering to proceed to the output if the current component
 * matches any of the given ids, otherwise simply does not send anything to the
 * output.
 * 
 * @since 1.1
 * @author Arjan Tijms
...
public class ConditionalResponseWriter extends ResponseWriterWrapper {
    ...
    private final Map<String, Boolean> renderedIdCache = new HashMap<String, Boolean>();
    private final Map<UIComponent, Boolean> renderedReferenceCache = new HashMap<UIComponent, Boolean>();

    public ConditionalResponseWriter(final ResponseWriter responseWriter,
    ...
    private boolean isForRenderedComponent() {
        final UIComponent currentComponent = UIComponent.getCurrentComponent(facesContext);

        // Typically a single component writes multiple times to the response
        // writer in quick succession.
        // Shortcut id matching by doing a cheaper check on the outcome of the
        // last processed component.
        ...
        // Check if a rendering decision already made for this component by
        // checking the cache
        ...
        lastRendered = componentIds.contains(currentComponent.getId())
            || clientIds.contains(currentComponent.getClientId());

        if (renderChildren) {
            // If current component not rendered because of explicit id match,
            // check if parent is rendered.
            if (!lastRendered) {
                ...
                // Explicitly rendered component, remember this by reference,
                // since client-id can change even for components
                // that aren't in an iterating naming container (e.g. UIData
                // changes its own client-id during iteration)
                renderedReferenceCache.put(currentComponent, lastRendered);
            }

            // Also remember client-id, in addition to the component reference
            // since iterating is often implemented by swapping the state and
            // identity
            // from the same component instance. So components with the same
            // object identity can have different component identities.
    ...
    @Override
    public void writeText(final Object text, final UIComponent component, final String property) throws IOException {
        if (isForRenderedComponent()) {
