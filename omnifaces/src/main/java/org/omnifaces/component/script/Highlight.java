...
/**
 * <p>
 * <strong>Highlight</strong> is a helper component which highlights all invalid
 * {@link UIInput} components and the associated labels by adding an error style
 * class to them. Additionally, it by default focuses the first invalid
 * {@link UIInput} component. The <code>&lt;o:highlight /&gt;</code> component
 * can be placed anywhere in the view, as long as there's only one of it.
 ...
 * </pre>
 * <p>
 * The default error style class name is <code>error</code>. You need to specify
 * a CSS style associated with the class yourself. For example,
 * 
 * <pre>
 * label.error {
 *   color: #f00;
 ...
 * </pre>
 * <p>
 * You can override the default error style class by the <code>styleClass</code>
 * attribute:
 * 
 * <pre>
 ...
 * </pre>
 * <p>
 * You can disable the default focus on the first invalid input element setting
 * the <code>focus</code> attribute.
 * 
 * <pre>
...
@ResourceDependencies({ @ResourceDependency(library = "javax.faces", name = "jsf.js", target = "head"), // Required
                                                                                                        // for
                                                                                                        // jsf.ajax.addOnEvent.
@ResourceDependency(library = "omnifaces", name = "omnifaces.js", target = "head") // Specifically
                                                                                   // highlight.js.
})
public class Highlight extends OnloadScript {
    ...
    public void encodeChildren(final FacesContext context) throws IOException {
        final UIOutput body = (UIOutput) getChildren().get(0);
        body.setValue(null); // Reset any previous value.

        if (context.isPostback() && context.isValidationFailed()) {
            final UIForm form = Components.getCurrentForm();

            if (form != null) {
                final StringBuilder clientIdsAsJSON = new StringBuilder();
                form.visitTree(VisitContext.createVisitContext(context, null, Highlight.VISIT_HINTS),
                               new VisitCallback() {
                                   ...
                                   public VisitResult visit(final VisitContext context, final UIComponent component) {
                                       if (component instanceof UIInput && !((UIInput) component).isValid()) {
                                           ...
                                           final String clientId = component.getClientId(context.getFacesContext());
                ...
                if (clientIdsAsJSON.length() > 0) {
                    body.setValue(String.format(Highlight.SCRIPT, clientIdsAsJSON, getStyleClass(), isFocus()));
                }
            }
        }
    ...
    public String getStyleClass() {
        return state.get(PropertyKeys.styleClass, Highlight.DEFAULT_STYLECLASS);
    }
    ...
    public Boolean isFocus() {
        return state.get(PropertyKeys.focus, Highlight.DEFAULT_FOCUS);
    }
