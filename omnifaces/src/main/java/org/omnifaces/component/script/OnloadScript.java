...
/**
 * <strong>OnloadScript</strong> is an extension to
 * <code>&lt;h:outputScript&gt;</code> which will be executed in the end of the
 * HTML body (thus when all HTML elements are initialized in the HTML DOM tree)
 * and will re-execute its script body on every ajax request. This is
 * particularly useful if you want to re-execute a specific helper script to
 * manipulate the HTML DOM tree, such as (re-)adding fancy tooltips, performing
 * highlights, etcetera, also after changes in the HTML DOM tree on ajax
 * responses.
 * <p>
 * You can put it anywhere in the view, it will always be relocated to the end
 * of body.
 * 
 * <pre>
...
public class OnloadScript extends ScriptFamily implements SystemEventListener {
    ...
    /**
     * Returns <code>true</code> if the given source is an instance of
     * {@link OnloadScript} or {@link UIViewRoot}.
     */
    @Override
    ...
    /**
     * If the event is a {@link PostAddToViewEvent}, then relocate the component
     * to end of body, so that we can make sure that the script is executed
     * after all HTML DOM elements are been created. Else if the event is a
     * {@link PostRestoreStateEvent} and the current request is an ajax request,
     * then subscribe to the {@link PreRenderViewEvent} event.
     */
    @Override
    ...
    /**
     * If the event is a {@link PreRenderViewEvent} and the current request is
     * an ajax request and this component is rendered and there are any
     * children, then encode the children as {@link Ajax#oncomplete(String...)}.
     */
    @Override
    public void processEvent(final SystemEvent event) throws AbortProcessingException {
        ...
        final String encoding = context.getExternalContext().getRequestCharacterEncoding();
        ...
        popComponentFromEL(context);
        final String script = buffer.toString().trim();
