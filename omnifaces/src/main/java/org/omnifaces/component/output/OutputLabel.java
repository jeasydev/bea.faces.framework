...
/**
 * <strong>OutputLabel</strong> is a component that extends the standard
 * {@link HtmlOutputLabel} and provides support for automatically setting its
 * value as the label of the component identified by its <code>for</code>
 * attribute (if any).
 * <p>
 * You can use it the same way as <code>&lt;h:outputLabel&gt;</code>, you only
 * need to change <code>h:</code> into <code>o:</code>.
 * 
 * @author Arjan Tijms
 */
    ...
    public void processEvent(final ComponentSystemEvent event) throws AbortProcessingException {
        if (event instanceof PostRestoreStateEvent) {
            ...
            if (!Utils.isEmpty(forValue)) {
                final UIComponent forComponent = Components.findComponentRelatively(this, forValue);

                if (forComponent == null) {
                    throw new IllegalArgumentException(String.format(OutputLabel.ERROR_FOR_COMPONENT_NOT_FOUND,
                                                                     forValue, getId()));
                }

                // To be sure, check if the target component doesn't have a
                // label already. This
                // is unlikely, since otherwise people have no need to use this
