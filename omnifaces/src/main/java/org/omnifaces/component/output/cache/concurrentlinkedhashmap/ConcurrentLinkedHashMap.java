...
public final class ConcurrentLinkedHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, Serializable {
    ...
    public static final class Builder<K, V> {
        ...
        public Builder() {
            ...
            initialCapacity = Builder.DEFAULT_INITIAL_CAPACITY;
            concurrencyLevel = Builder.DEFAULT_CONCURRENCY_LEVEL;
        ...
        public ConcurrentLinkedHashMap<K, V> build() {
            ConcurrentLinkedHashMap.checkState(capacity >= 0);
         ...
         *            threads
         * @throws IllegalArgumentException if the concurrencyLevel is less than
         *             or equal to zero
         */
         ...
         * 
         * @param initialCapacity the initial capacity used to size the hash
         *            table to accommodate this many entries.
        ...
        public Builder<K, V> weigher(final Weigher<? super V> weigher) {
            this.weigher = (weigher == Weighers.singleton())
                                                            ? Weighers.<K, V> entrySingleton()
                                                            : new BoundedEntryWeigher<K, V>(Weighers
                                                                .asEntryWeigher(weigher));
            return this;
        }
    }
    ...
    final class EntryIterator implements Iterator<Entry<K, V>> {
        ...
        @Override
        public void remove() {
            ConcurrentLinkedHashMap.checkState(current != null);
    ...
    final class Node extends AtomicReference<WeightedValue<V>> implements Linked<Node> {
        ...
        void makeDead() {
            for (;;) {
                ...
                final WeightedValue<V> dead = new WeightedValue<V>(current.value, 0);
    ...
    static {
        final int buffers = ConcurrentLinkedHashMap.ceilingNextPowerOfTwo(Runtime.getRuntime().availableProcessors());
        AMORTIZED_DRAIN_THRESHOLD = (1 + buffers) * ConcurrentLinkedHashMap.BUFFER_THRESHOLD;
    ...
    private ConcurrentLinkedHashMap(final Builder<K, V> builder) {
        ...
        capacity = Math.min(builder.capacity, ConcurrentLinkedHashMap.MAXIMUM_CAPACITY);
        ...
        buffers = (Queue<Task>[]) new Queue[ConcurrentLinkedHashMap.NUMBER_OF_BUFFERS];
        bufferLengths = new AtomicIntegerArray(ConcurrentLinkedHashMap.NUMBER_OF_BUFFERS);
        ...
        pendingNotifications = (listener == DiscardingListener.INSTANCE)
                                                                        ? (Queue<Node>) ConcurrentLinkedHashMap.DISCARDING_QUEUE
    ...
    void afterCompletion(final Task task) {
        ...
        if (shouldDrainBuffers(delayable)) {
            tryToDrainBuffers(ConcurrentLinkedHashMap.AMORTIZED_DRAIN_THRESHOLD);
        }
        notifyListener();
    }
    ...
    Set<K> orderedKeySet(final boolean ascending, final int limit) {
        ...
        evictionLock.lock();
        try {
            drainBuffers(ConcurrentLinkedHashMap.AMORTIZED_DRAIN_THRESHOLD);

            final int initialCapacity = (weigher == Weighers.singleton()) ? Math.min(limit, (int) weightedSize()) : 16;
    ...
    private void readObject(@SuppressWarnings("unused") final ObjectInputStream stream) throws InvalidObjectException {
    ...
    boolean schedule(final Task task) {
        final int index = ConcurrentLinkedHashMap.bufferIndex();
        ...
        // level
        if (buffered <= ConcurrentLinkedHashMap.MAXIMUM_BUFFER_SIZE) {
            buffers[index].add(task);
            return (buffered <= ConcurrentLinkedHashMap.BUFFER_THRESHOLD);
    ...
    public void setCapacity(final long capacity) {
        ...
        evictionLock.lock();
        try {
            this.capacity = Math.min(capacity, ConcurrentLinkedHashMap.MAXIMUM_CAPACITY);
            drainBuffers(ConcurrentLinkedHashMap.AMORTIZED_DRAIN_THRESHOLD);
            evict();
        } finally {
            evictionLock.unlock();
