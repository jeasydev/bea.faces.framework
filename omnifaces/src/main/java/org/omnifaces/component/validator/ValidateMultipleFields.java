...
/**
 * Base class which is to be shared between all multi field validators. The
 * implementors have to call the super constructor with the default message. The
 * implementors have to override the
 * {@link #validateValues(FacesContext, List, List)} method to perform the
 * actual validation.
 * <hr>
 * <h3>General usage of all multiple field validators</h3>
 * <p>
 * This validator must be placed inside the same <code>UIForm</code> as the
 * <code>UIInput</code> components in question. The <code>UIInput</code>
 * components must be referenced by a space separated collection of their client
 * IDs in the <code>components</code> attribute. This validator can be placed
 * anywhere in the form, but keep in mind that the components will be validated
 * in the order as they appear in the form. So if this validator is been placed
 * before all of the components, then it will be executed before any of the
 * component's own validators. If this validator fails, then the component's own
 * validators will not be fired. If this validator is been placed after all of
 * the components, then it will be executed after any of the component's own
 * validators. If any of them fails, then this validator will not be exeucted.
 * It is not recommended to put this validator somewhere in between the
 * referenced components as the resulting behaviour may be confusing. Put this
 * validator either before or after all of the components, depending on how you
 * would like to prioritize the validation.
 * 
 * <pre>
 ...
 * 
 * <p>
 * In an invalidating case, all of the referenced components will be marked
 * invalid and a faces message will be added on the client ID of this validator
 * component. The default message can be changed by the <code>message</code>
 * attribute. Any "{0}" placeholder in the message will be substituted with a
 * comma separated string of labels of the referenced input components.
 * 
 * <pre>
 ...
 * 
 * <p>
 * The faces message can also be shown for all of the referenced components
 * using <code>showMessageFor="@all"</code>.
 * 
 * <pre>
 ...
 * Other values than <code>@this</code> or <code>@all</code> are not allowed.
 * <p>
 * The validator can be disabled by the <code>disabled</code> attribute. It
 * accepts a request based EL expression.
 * 
 * <pre>
 ...
 * 
 * <p>
 * There is a read-only <code>validationFailed</code> attribute which can be
 * used to determine if the validation by this component has failed.
 * 
 * <pre>
...
public abstract class ValidateMultipleFields extends ValidatorFamily {
     ...
     */
    public ValidateMultipleFields(final String defaultMessage) {
     ...
     * @throws IllegalArgumentException When the <code>components</code>
     *             attribute is missing, or when it references an non-existing
     *             component, or when it references a non-input component.
     */
    protected List<UIInput> collectComponents() {
        final String components = getComponents();

        if (components.isEmpty()) {
            throw new IllegalArgumentException(String.format(ValidateMultipleFields.ERROR_MISSING_COMPONENTS,
                                                             getClass().getSimpleName()));
        }

        final UIComponent namingContainerParent = getNamingContainer();
        final List<UIInput> inputs = new ArrayList<UIInput>();

        for (final String clientId : components.split("\\s+")) {
            final UIComponent found = namingContainerParent.findComponent(clientId);

            if (found == null) {
                throw new IllegalArgumentException(String.format(ValidateMultipleFields.ERROR_UNKNOWN_COMPONENT,
                                                                 getClass().getSimpleName(), clientId));
            } else if (!(found instanceof UIInput)) {
                throw new IllegalArgumentException(String.format(ValidateMultipleFields.ERROR_INVALID_COMPONENT,
                                                                 getClass().getSimpleName(), clientId, found.getClass()
                                                                     .getName()));
            }

            final UIInput input = (UIInput) found;
     ...
     */
    protected List<Object> collectValues(final List<UIInput> inputs) {
        final List<Object> values = new ArrayList<Object>(inputs.size());

        for (final UIInput input : inputs) {
    ...
    public String getShowMessageFor() {
        return state.get(PropertyKeys.showMessageFor, ValidateMultipleFields.DEFAULT_SHOWMESSAGEFOR);
    }
    ...
    public Boolean isDisabled() {
        return state.get(PropertyKeys.disabled, ValidateMultipleFields.DEFAULT_DISABLED);
    }
     ...
     * 
     * @param components The client identifiers of components which are to be
     *            validated.
     */
     ...
     * 
     * @param showMessageFor The client identifier to show the validation
     *            message for.
     */
    public void setShowMessageFor(final String showMessageFor) {
     ...
     */
    protected void showMessage(final FacesContext context, final List<UIInput> inputs) {
        final StringBuilder labels = new StringBuilder();

        for (final Iterator<UIInput> iterator = inputs.iterator(); iterator.hasNext();) {
        ...
        final String showMessageFor = getShowMessageFor();
        ...
        if (showMessageFor.equals("@this")) {
            ...
            throw new IllegalArgumentException(String.format(ValidateMultipleFields.ERROR_INVALID_SHOWMESSAGEFOR,
                                                             getClass().getSimpleName(), showMessageFor));
        }
    }
     ...
     */
    @Override
    protected void validateComponents(final FacesContext context) {
        if (isDisabled()) {
            return;
        }

        final List<UIInput> inputs = collectComponents();

        if (inputs.isEmpty()) {
            return;
        }

        final List<Object> values = collectValues(inputs);
     ...
     * 
     * @throws IllegalArgumentException When there is no parent of type
     *             {@link UIForm}, or when there are any children.
     */
    @Override
    ...
    /**
     * Perform the validation on the collected values of the input components
     * and returns whether the validation is successful.
     * 
     * @param context The faces context to work with.
     ...
     * @return <code>true</code> if validation is successful, otherwise
     *         <code>false</code> (and thus show the message).
     */
