...
public class MethodParam extends TagHandler {
    ...
    public MethodParam(final TagConfig config) {
        super(config);
    ...
    public void apply(final FaceletContext ctx, final UIComponent parent) throws IOException {
        ...
        // The original value expression we get inside the Facelets tag, that's
        // actually the method expression passed-in by the user.
        final ValueExpression valueExpression = value.getValueExpression(ctx, Object.class);

        // A method expression that wraps the value expression and uses its own
        // invoke method to get the value from the wrapped expression.
        final MethodExpression methodExpression = new MethodExpressionValueExpressionAdapter(valueExpression);
        ...
        // JUEL older than 2.2.6 had the expectation that the value expression
        // only wraps a Method instance that can be statically called.
        if (Hacks.isJUELUsed(ctx.getExpressionFactory()) && !Hacks.isJUELSupportingMethodExpression()) {
            ...
            // Sun/Apache EL and JUEL 2.2.6 and higher can call a method
            // expression wrapped in a value expression.
            valueExpressionWrapper = ctx.getExpressionFactory().createValueExpression(methodExpression,
