...
/**
 * The <code>&lt;o:validateBean&gt;</code> allows the developer to control bean
 * validation on a per-{@link UICommand} or {@link UIInput} component basis. The
 * standard <code>&lt;f:validateBean&gt;</code> only allows that on a per-form
 * or a per-request basis (by using multiple tags and conditional EL expressions
 * in its attributes) which may end up in boilerplate code.
 * <p>
 * Usage examples:
 * 
 * <pre>
...
public class ValidateBean extends TagHandler {
     ...
     */
    public ValidateBean(final TagConfig config) {
        super(config);
    ...
    public void apply(final FaceletContext context, final UIComponent parent) throws IOException {
        if (!(parent instanceof UICommand || parent instanceof UIInput)) {
            throw new IllegalArgumentException(ValidateBean.ERROR_INVALID_PARENT);
        }
        ...
        Events.addBeforePhaseListener(PhaseId.PROCESS_VALIDATIONS, new Callback.Void() {
            ...
            public void invoke() {
                if (Components.hasInvokedSubmit(parent)) {
                    final SystemEventListener listener = new BeanValidationEventListener(validationGroups, disabled);
