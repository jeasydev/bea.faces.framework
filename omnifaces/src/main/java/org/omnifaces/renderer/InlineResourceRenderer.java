...
public abstract class InlineResourceRenderer extends Renderer {
    ...
    /**
     * Obtain the resource, construct a {@link Reader} around it using the
     * character encoding as obtained from the response writer and then invoke
     * {@link #startElement(ResponseWriter, UIComponent)},
     * {@link #writeResource(Reader, ResponseWriter)} and
     * {@link #endElement(ResponseWriter)} in sequence.
     */
    @Override
    public void encodeChildren(final FacesContext context, final UIComponent component) throws IOException {
        final ResponseWriter writer = context.getResponseWriter();
        final String name = (String) component.getAttributes().get("name");
        final String library = (String) component.getAttributes().get("library");
        final Resource resource = context.getApplication().getResourceHandler().createResource(name, library);
        final Reader reader = new InputStreamReader(resource.getInputStream(), writer.getCharacterEncoding());
     ...
     * 
     * @param writer The response writer.
     * @param component The {@link UIComponent} to which this element
     *            corresponds.
     * @throws IOException When an I/O error occurs.
     */
     ...
     * @param writer The response writer where the resource content has to be
     *            written to.
     * @throws IOException When an I/O error occurs.
     */
