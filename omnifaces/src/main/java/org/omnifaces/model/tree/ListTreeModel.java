...
/**
 * A concrete implementation of {@link TreeModel} which holds the tree children
 * in a {@link List}.
 * 
...
public class ListTreeModel<T> implements TreeModel<T> {
    ...
    @Override
    public TreeModel<T> addChild(final T data) {
        final ListTreeModel<T> child = new ListTreeModel<T>();
        child.data = data;
        return addChildNode(child);
    }

    @Override
    public TreeModel<T> addChildNode(final TreeModel<T> child) {
    ...
    public boolean equals(final Object object) {
        ...
        // Property checks.
        final ListTreeModel other = (ListTreeModel) object;
     ...
     */
    private TreeModel<T> getNextSibling(final TreeModel<T> parent, final int index) {
        if (parent == null) {
            return null;
     ...
     */
    private TreeModel<T> getPreviousSibling(TreeModel<T> parent, final int index) {
        if (parent == null) {
            return null;
        } else if (index >= 0) {
            return parent.getChildren().get(index);
    ...
    public TreeModel<T> remove() {
        if (parent != null) {
            synchronized (parent.children) {
                ...
                // Fix the indexes of the children (that's why it needs to be
                // synchronized).
